  ...
  return (
    <div className="flex flex-col gap-10 lg:px-16 px-5 py-14 bg-black text-white">
      <div className="flex flex-col items-center justify-center gap-2">
        ...
        <p className="sm:text-4xl text-3xl text-center sm:text-start font-extrabold space-grotesk">
          Explore The <span className="text-froozen">Flavour Range</span>
        </p>
      </div>
      {/* cards */}
      <div className="grid lg:grid-cols-3 sm:grid-cols-2 grid-cols-1 items-center justify-center gap-5 w-full">
        {cardsData.map((item, index) => (
          <div
            key={index}
            className="group relative w-auto h-[400px] overflow-hidden rounded-t-xl cursor-pointer"
          >
            {/* Background Image */}
            <img
              src={item.img}
